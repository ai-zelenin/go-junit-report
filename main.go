...
var (
	...
	unitConfig       = flag.String("cfg-file", ".unit-cfg.yml", "required coverage exclude file")
	requiredCoverage = flag.Float64("rcover", 0, "set required coverage")
...
func main() {
	...
	var unitCfg = make(map[string]*gtr.UnitCfg)
	if *unitConfig != "" {
		data, err := os.ReadFile(*unitConfig)
		if err != nil {
			exitf(err.Error())
		}
		err = yaml.Unmarshal(data, &unitCfg)
		if err != nil {
			exitf(err.Error())
		}
	}
	for pkg, cfg := range unitCfg {
		fmt.Printf("%s %#v\n", pkg, cfg)
	}
	...
	if *setExitCode && !report.IsSuccessful(unitCfg) {
		os.Exit(1)
	}
}
